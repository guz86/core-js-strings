...
function removeFirstOccurrences(str, value) {
  ...
  const firstString = str.substring(0, index);
  let lastString = str;
  if (index !== -1) {
    lastString = str.substring(index + value.length, str.length);
  }
...
function countVowels(str) {
  return str.split('').reduce((count, item) => {
    if ('aeiouy'.includes(item.toLowerCase())) {
      return count + 1;
    }
    return count;
  }, 0);
...
function isPalindrome(str) {
  const tempStr = str
    ...
    .reverse()
    .join('');
  const tempStr2 = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
...
function extractNameFromTemplate(value) {
  const result = value.replace(/[^a-zA-Zа-яА-Я ]/g, '').split(' ');
  ...
  return str
    ...
    .map((char) => {
      ...
      if (isUpperCase || isLowerCase) {
        const сode = isUpperCase ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
        return String.fromCharCode(
          ((char.charCodeAt(0) - сode + 13) % 26) + сode
        );
      }
      return char;
    })
